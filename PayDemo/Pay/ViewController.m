...
//    NSArray *productIds = [NSArray arrayWithContentsOfURL:plistURL];
    //self.profuctIdArr = @[@"ceshi"];
    self.profuctIdArr = @[@"com.21cn.cloud189.vip1.test"];
...
-(void)verifyPurchaseWithPaymentTransaction{
    ...
    NSString *bodyString = [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\",\"password\" : \"%@\"}", receiptString,@"fda0eb3c1c0d4433aa84dd4018906bab"];//拼接请求数据
    ...
    if([dic[@"status"] intValue]==0){
        ...
        if ([productIdentifier isEqualToString:@"com.21cn.cloud189.vip1.test"]) {
    ...
    for(SKPaymentTransaction *tran in transaction){
        ...
        switch (tran.transactionState) {
            case SKPaymentTransactionStatePurchased:{
                ...
                {
                    NSLog(@"交易完成 续订商品ID：%@",tran.originalTransaction.payment.productIdentifier);
                    [self verifyPurchaseWithPaymentTransaction];
                    [[SKPaymentQueue defaultQueue] finishTransaction:tran.originalTransaction];
                    NSLog(@"我是续订任务");
                }
                else
                {
                    NSLog(@"交易完成 商品ID：%@",tran.payment.productIdentifier);
                    [self verifyPurchaseWithPaymentTransaction];
